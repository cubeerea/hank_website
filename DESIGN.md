...
### Navigation
- Sticky, mono labels in Soft Ink at 0.8125rem (bumped up from 0.75rem for legibility); hover shifts to Deep Ink with a 1.5px Terracotta underline; active (current section via scroll-spy) shifts to Deep Ink, 500-weight, and sits inside the morphing pill below.
- **The underline is directional and hover-only.** It wipes in from the left on hover and out toward the right on leave — one rule with `transform-origin` flipped between the two states. A centre-out scale reads as a bar switching on; the wipe reads as a pen stroke. Same treatment on `.nav-mark`, drawn in ink rather than terracotta since the wordmark is a destination, not a section marker. It no longer parks at full width for `.is-active` — that job moved to the pill, so the two indicators never compete for the same link at once.
- **The morphing active pill.** `.nav-pill`, a Terracotta-tint (`--color-tint`) capsule absolutely positioned inside `.nav-list`, slides and resizes via `left`/`width` transitions to sit behind whichever link `initNavTracking()` marks active — the same scroll-spy pass that used to only toggle the underline. Padded 8px past the link's own box on each side so it reads as a pill around the label, not a tight box on it. Starts at `width: 0; opacity: 0` so there's nothing to animate in from until JS has measured the active link once; `z-index: 0` under the links (`z-index: 1`) so label text always paints on top. Below 720px, Home and Stack's `<li>`s go `display: none` but stay in the scroll-spy's target list, so the "active" section can still resolve to one of them — `positionPill()` checks `link.offsetParent === null` and hides the pill rather than positioning it against a hidden element's zeroed-out rect.
